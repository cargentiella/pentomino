...
def solve_pentomino_cube(width, height, depth):

	if width * height * depth != 60:
		print("[ERROR] width * height * depth not equal 60.")
...
if __name__ == '__main__':
	...
	usage = 'Usage: python solve_pentomino_rect [width] [height] [depth]'
	...
	if len(args) == 4:
		...
		block = BLOCK()
		file = "solution_cube_" + str(width) + "x" + str(height) + "x" + str(depth) + ".txt"

		f = open(file, 'w', encoding='utf-8')
		writer = csv.writer(f, delimiter=',', lineterminator='\n')

		solve_pentomino_cube(width, height, depth)
